...
genotypes <- sort(unique(s2c[, genovar]))
print(genotypes)

print('#Reading Kallisto results')
s2c <- dplyr::select(s2c, sample= sample, genovar)
print(s2c)
...
so <- sleuth_prep(s2c, ~get('genovar'), target_mapping= t2g)
print(so)
so <- sleuth_fit(so, ~get('genovar'), fit_name = 'full')
